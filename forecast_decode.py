...
for i in range(no_samples):
    surface_file = os.path.join(forecast_dir, "output_surface_"+str(i)+".npy")
    upper_file = os.path.join(forecast_dir, "output_upper_"+str(i)+".npy")
...
# Load the saved numpy arrays
    ...
    with nc.Dataset(
        os.path.join(forecast_dir, "output_surface_"+str(i)+".nc"), "w", format="NETCDF4_CLASSIC"
    ) as nc_file:
        # Create dimensions
    ...
    with nc.Dataset(
        os.path.join(forecast_dir, "output_upper_"+str(i)+".nc"), "w", format="NETCDF4_CLASSIC"
    ) as nc_file:
        # Create dimensions
        nc_file.createDimension("longitude", 1440)
        nc_file.createDimension("latitude", 721)
